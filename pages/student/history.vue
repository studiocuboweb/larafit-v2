              ...
              <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                ...
                >
                  <svg
                    ...
                    :class="expandedSessions.has(session.date) ? 'transform rotate-180' : ''"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
    ...
    <div v-if="exercise.notes" class="mt-3 p-3 bg-blue-50 rounded-lg border-l-4 border-blue-400">
      <p class="text-sm text-gray-700">
        ...
        <div v-if="workoutSessions.length === 0" class="text-center py-12">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">Nenhum treino realizado</h3>
          <p class="mt-1 text-sm text-gray-500">Comece a treinar para ver seu histórico aqui!</p>
          <div class="mt-6">
            <NuxtLink
              to="/student/workouts"
...
const totalDuration = computed(() => {
  return workoutSessions.value.reduce((sum, session) => sum + session.duration, 0)
})
